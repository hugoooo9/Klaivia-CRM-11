...
const SERVICE_TABS: { id: Service; label: string; icon: typeof Globe; desc: string }[] = [
  { id: "web", label: "Site web", icon: Globe, desc: "Proposer un nouveau site" },
...
export function ApproachEmailModal({
  ...
  // À l'ouverture : si initialDraft → l'utiliser, sinon générer auto avec service par défaut
  useEffect(() => {
    if (!open) return;
    if (initialDraft) {
  ...
  return (
    ...
    <Dialog open={open} onOpenChange={(v) => !busy && onOpenChange(v)}>
      <DialogContent className="max-h-[90vh] max-w-2xl overflow-y-auto bg-card">
        ...
        {/* Sélecteur de service Klaivia à proposer */}
        {!initialDraft && (
